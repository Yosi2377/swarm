# Auto-Generated Skill: status
# Generated from 5 lessons
# Pattern detected: "status" appeared 13 times

## Lessons Learned
- Add a cleanup job or query filter: if event.commenceTime < now and status is not completed, auto-mark or filter out.
...
## Rules
When encountering situations related to "status":
- Add a cleanup job or query filter: if event.commenceTime < now and status is not completed, auto-mark or filter out.
- 🔍 Evaluating thread 2809 for agent koder...
📝 Auto-committing 5 workspace files...
[master 658970f] #2809: auto-commit workspace changes (evaluator)
