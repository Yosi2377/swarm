...
# Example: pieces-realtime.sh "telegram:yossi" "תתקן את הבאג בהימורים"
...
# Save via Pieces CLI (fastest method)
export DISPLAY=:1
echo "// [$TIMESTAMP] $SOURCE
$CONTENT" | xclip -selection clipboard 2>/dev/null
echo "y" | pieces --ignore-onboarding create 2>/dev/null > /dev/null

# Also append to daily buffer for batch context
