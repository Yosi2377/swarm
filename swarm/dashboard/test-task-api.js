...
async function runTests() {
  ...
  // GET /api/tasks/managed — list
  await test('GET /api/tasks/managed returns tasks array', async () => {
    const r = await req('GET', '/api/tasks/managed');
    assert.strictEqual(r.status, 200);
    assert(Array.isArray(r.data.tasks), 'tasks should be array');
    assert(typeof r.data.total === 'number', 'total should be number');
  });

  // POST /api/tasks/managed — create
  await test('POST /api/tasks/managed creates task', async () => {
    const r = await req('POST', '/api/tasks/managed', { agent_id: 'tester', thread_id: '99999', task_desc: 'Test task from API tests', project: 'test' });
  ...
  await test('POST duplicate returns 409', async () => {
    const r = await req('POST', '/api/tasks/managed', { agent_id: 'tester', thread_id: '99999', task_desc: 'dup' });
    assert.strictEqual(r.status, 409);
  });

  // POST missing fields
  await test('POST missing fields returns 400', async () => {
    const r = await req('POST', '/api/tasks/managed', { agent_id: 'tester' });
    assert.strictEqual(r.status, 400);
  });

  // GET /api/tasks/managed/:id
  await test('GET /api/tasks/managed/:id returns task', async () => {
    const r = await req('GET', `/api/tasks/managed/${TEST_TASK_ID}`);
    assert.strictEqual(r.status, 200);
    assert.strictEqual(r.data.id, TEST_TASK_ID);
  ...
  await test('GET nonexistent returns 404', async () => {
    const r = await req('GET', '/api/tasks/managed/nonexistent-999');
    assert.strictEqual(r.status, 404);
  });

  // POST retry
  await test('POST /api/tasks/managed/:id/retry retries task', async () => {
    const r = await req('POST', `/api/tasks/managed/${TEST_TASK_ID}/retry`);
    assert.strictEqual(r.status, 200);
  ...
  // POST cancel
  await test('POST /api/tasks/managed/:id/cancel cancels task', async () => {
    const r = await req('POST', `/api/tasks/managed/${TEST_TASK_ID}/cancel`);
    assert.strictEqual(r.status, 200);
  ...
  await test('GET with status filter works', async () => {
    const r = await req('GET', '/api/tasks/managed?status=cancelled');
    assert.strictEqual(r.status, 200);
  ...
  await test('GET with agent filter works', async () => {
    const r = await req('GET', '/api/tasks/managed?agent=tester');
    assert.strictEqual(r.status, 200);
